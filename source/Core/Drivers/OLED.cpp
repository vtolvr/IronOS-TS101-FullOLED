...
void i2c_send_command_byte(unsigned char cmd) {
  uint8_t buf[] = {0x0, cmd};
  I2C_CLASS::I2C_RegisterWrite(DEVICEADDR_OLED, 0x00,cmd);
}
...
void OLED::refresh() {
  if (checkDisplayBufferChecksum()) {
    ...
    oled_bulk_write(0, 0, 128, 32, screenBuffer + FRAMEBUFFER_START);
